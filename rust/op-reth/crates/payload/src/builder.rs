...
enum StateRootWaitResult {
    Task(Result<StateRootComputeOutcome, ParallelStateRootError>),
    Timeout(Duration),
    Disconnected,
}
...
///
/// `None` preserves the previous unbounded wait. A timeout becomes an error so the caller takes
/// the existing synchronous fallback path.
fn await_sparse_trie_state_root(
    handle: &mut StateRootHandle,
    ...
    let result = wait.map_or_else(
        || {
            receiver
                .recv()
                .map_or_else(|_| StateRootWaitResult::Disconnected, StateRootWaitResult::Task)
        },
        |wait| match receiver.recv_timeout(wait) {
            ...
            Err(RecvTimeoutError::Disconnected) => StateRootWaitResult::Disconnected,
        },
    );
    ...
    match result {
        ...
        StateRootWaitResult::Disconnected => {
...
impl<Txs> OpBuilder<'_, Txs> {
        ...
        Attrs: OpAttributes<Transaction = N::SignedTx>,
    {
        ...
        let BlockBuilderOutcome {
            ...
            // final root is usually immediate. Bound the wait anyway because a silent task would
            // otherwise park the payload job and `getPayload` indefinitely.
            match await_sparse_trie_state_root(&mut handle, ctx.builder_config.state_root_wait) {
                Ok(outcome) => {
                ...
                Err(err) => {
                    ...
                    // Drop the handle before starting the fallback. Reth versions with a
                    // cancellation guard can then stop work whose result is no longer needed.
                    drop(handle);
