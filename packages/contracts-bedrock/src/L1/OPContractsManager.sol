...
contract OPContractsManagerDeployer is OPContractsManagerBase {
        ...
        returns (OPContractsManager.DeployOutput memory)
    {
        ...
        data = encodeOptimismPortalInitializer(output, ISuperchainConfig(address(output.celoSuperchainConfigProxy)));
        upgradeToAndCall(
        ...
        data = encodeL1StandardBridgeInitializer(output, ISuperchainConfig(address(output.celoSuperchainConfigProxy)));
        upgradeToAndCall(
            output.opChainProxyAdmin, address(output.l1StandardBridgeProxy), implementation.l1StandardBridgeImpl, data
        );

        data = encodeDelayedWETHInitializer(_input, ISuperchainConfig(address(output.celoSuperchainConfigProxy)));
        // Eventually we will switch from DelayedWETHPermissionedGameProxy to DelayedWETHPermissionlessGameProxy.
        upgradeToAndCall(
            output.opChainProxyAdmin,
    ...
    function encodeCeloSuperchainConfigInitializer(
        OPContractsManager.DeployInput memory _input,
        ISuperchainConfig _superchainConfig
    )
        internal
