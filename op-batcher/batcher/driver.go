...
type BatchSubmitter struct {
	...
	// authGroup tracks the fallback and TEE batchers' receipt-watcher
	// goroutines (one per auth+batch pair) so the publishing loop can drain
	// them via waitForAuthGroup before closing receiptsCh. It is intentionally
	// unbounded: pending-tx throttling is enforced by the txmgr Queue
	// (queue.Send blocks at MaxPendingTransactions), and the live watcher count
	// is derived from the queue's in-flight tx count, so it inherits the same
	// bound.
