...
echo forge script scripts/Deploy.s.sol:Deploy --private-key $ADMIN_PRIVKEY --broadcast --rpc-url $L1_RPC && \
echo forge script scripts/Deploy.s.sol:Deploy --sig 'sync()' --private-key $ADMIN_PRIVKEY --broadcast --rpc-url $L1_RPC \
| tee deploy.log
