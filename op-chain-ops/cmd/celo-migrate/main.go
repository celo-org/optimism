...
func main() {
	...
	app := &cli.App{
		...
		Action: func(ctx *cli.Context) error {
			...
			if err := jsonutil.WriteJSON(rollupConfig, ioutil.ToStdOutOrFileOrNoop(outfileRollupConfig, OutFilePerm)); err != nil {
				return err
			}
...
func ApplyMigrationChangesToDB(genesis *core.Genesis, dbPath string, dbCache int, dbHandles int, commit bool) (*types.Header, error) {
	...
	for k, v := range genesis.Alloc {
		...
		db.SetBalance(k, uint256.MustFromBig(v.Balance), tracing.BalanceChangeUnspecified)
		db.SetCode(k, v.Code)
