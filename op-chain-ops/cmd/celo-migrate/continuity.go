...
func (r *RLPBlockRange) CheckContinuity() error {
	...
	for i := range r.hashes {
		...
		if currElement.Header().Number.Uint64() != r.start+uint64(i) {
			return fmt.Errorf("decoded header number mismatch indicating a gap in block numbers: expected %d, actual %d", r.start+uint64(i), currElement.Header().Number.Uint64())
		}
		if prevElement != nil {
...
func (r *RLPBlockRange) CheckLengths(expectedLength int) error {
	var err error
	if len(r.hashes) != expectedLength {
		err = fmt.Errorf("Unexpected number of hashes for block range: expected %d, actual %d", expectedLength, len(r.hashes))
	}
	if len(r.bodies) != expectedLength {
	...
	if len(r.headers) != expectedLength {
	...
	if len(r.receipts) != expectedLength {
