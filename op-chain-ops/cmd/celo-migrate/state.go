...
var (
	...
	celoTokenAddressMap = map[uint64]common.Address{
		alfajoresChainId: addresses.AlfajoresAddresses.CeloToken,
		mainnetChainId:   addresses.MainnetAddresses.CeloToken,
	}
)
...
func applyStateMigrationChanges(config *genesis.DeployConfig, genesis *core.Genesis, dbPath string, migrationBlockTime uint64) (*types.Header, error) {
	...
	newRoot, err := db.Commit(migrationBlock.Uint64(), true, true)
	if err != nil {
		return nil, err
	}
	...
	cel2Block := types.NewBlock(cel2Header, b, nil, trie.NewStackTrie(nil), types.DefaultBlockConfig)
