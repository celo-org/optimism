...
func TestCollectAuthenticatedBatchesBlockRefCache(t *testing.T) {
	...
	result, err := CollectAuthenticatedBatches(ctx, l1F, chain[200], authenticatorAddr, caches, logger)
	require.NoError(t, err)
	require.Len(t, result, 0)
	l1F.AssertExpectations(t)
	...
	result2, err := CollectAuthenticatedBatches(ctx, l1F2, chain[201], authenticatorAddr, caches, logger)
	require.NoError(t, err)
