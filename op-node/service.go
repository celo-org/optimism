...
// celoMainnetIsthmusTimestamp is celo mainnet's Isthmus activation time (Wed Jul 09 2025
// 15:00:00 GMT+0000). This used to be params.CeloMainnetIsthmusTimestamp in op-geth, but op-geth
// now loads celo fork times from the superchain registry and no longer exports the constant, so
// the value is inlined here.
const celoMainnetIsthmusTimestamp uint64 = 1752073200
...
func applyCeloHardforks(rollupConfig *rollup.Config) {
	switch bigs.Uint64Strict(rollupConfig.L2ChainID) {
	case params.CeloMainnetChainID:
		activationTime := celoMainnetIsthmusTimestamp
